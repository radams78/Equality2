...
subps : ∀ {n Γ Δ} {T : Typeover n Δ} {⟦ρ⟧ ⟦σ⟧ τs}
  ...
  {⟦t⟧} (t : Δ ⊢ T ∋ ⟦t⟧) →
  Γ ⊢ Typeover-eq τ T (record {
    vertex = λ γ → Section.vertex ⟦t⟧ (OneTypeMap.vertex ⟦ρ⟧ γ);
    edge = λ γ* → Section.edge ⟦t⟧ (ap₂' (OneTypeMap.edge ⟦ρ⟧) γ*);
    face = λ sq-fill → Section.face ⟦t⟧ (ap₃' (OneTypeMap.face ⟦ρ⟧) sq-fill)}) (record {
    vertex = λ γ → Section.vertex ⟦t⟧ (OneTypeMap.vertex ⟦σ⟧ γ);
    edge = λ γ* → Section.edge ⟦t⟧ (ap₂' (OneTypeMap.edge ⟦σ⟧) γ*);
    face = λ sq-fill → Section.face ⟦t⟧ (ap₃' (OneTypeMap.face ⟦σ⟧) sq-fill)}) ∋ record {
